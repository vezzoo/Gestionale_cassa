...
<body>
<div style=\"position: absolute; top: 0; left: 0; width: 180mm; background-color: #FFF; \">

    <div style=\"float: left;\"><h1 style=\"margin: 20px; margin-bottom: 1px\">$order_num</h1></div>
    <div style=\"float: right;\"><h1 style=\"float: right; margin: 20px; margin-bottom: 1px; margin-top: 8px\">CUCINA</h1></div>
    <table style=\"width: 90%; margin-left: 5%;\">
        ...
        foreach ($data as $i) {
            ...
            $qta = get_object_vars($i)["qta"];
            $gr = get_object_vars($i)["group"];
            if($gr == "CUCINA" || $gr == "MENU" ) $html .= "<tr class=\"piccolo \"><td>$nome</td><td class=\"chiudi\">$qta</td></tr>";
            $n++;
        }
...
if($_POST["notes"] != "")
$html .= "<h2>Note per la cucina:</h2><p>". $_POST["notes"] . "</p>";

    if($_POST["asporto"] == "true")
        $html .= "<h2>ASPORTO</h2>";

    
   $html .= " <p style=\"float: right; margin: 10px; margin-top: 4px;\" id=\"DATE_BRUTTO_MA_MOLTO\">" . date("d/m/Y H:i", time()) ."</p>
</div></body></html>";
